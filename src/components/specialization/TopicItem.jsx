...
const TopicItem = ({ topic, isExpanded, onToggle, moduleId }) => {
  ...
  const getModuleSlug = () => {
    if (!moduleId) return "html"
    const mk = String(moduleId).toLowerCase()
    if (mk === "s1") return "html"
    if (mk === "s2") return "css"
    return mk // already 'html' or 'css'
  }

  const handleTopicClick = () => {
    navigate(`/documentation-specialization/${getModuleSlug()}`)
  }

  const getBadgeLabel = (sub) => {
    return sub.difficulty || (sub._raw?.show_compiler ? "Compiler" : "Read")
  }
  const getProblemsCount = (sub) => {
    if (typeof sub.problems === "number") return sub.problems
    const len = sub._raw?.theory?.practise_problems?.length
    return typeof len === "number" ? len : 0
  }

  return (
    <div className="flex flex-col bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
      <div className="p-4 sm:p-6 cursor-pointer hover:bg-gray-800" onClick={handleTopicClick}>
        <div className="flex items-center justify-between">
          ...
          <div
            ...
            className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0 ml-2"
          >
          ...
          <div className="p-3 sm:p-4 space-y-2 sm:space-y-3">
            {topic.subtopics.map((subtopic, index) => (
              <div
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  navigate(`/documentation-specialization/${getModuleSlug()}/${topic.id}`)
                }}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs px-1 sm:px-2 py-1 bg-purple-900 text-purple-300 rounded-full flex-shrink-0">
                      {getBadgeLabel(subtopic)}
                    </span>
                  </div>
                  <p className="text-gray-400 text-xs sm:text-sm mt-1 line-clamp-2">{subtopic.description}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <span className="text-xs sm:text-sm text-gray-400">{getProblemsCount(subtopic)}p</span>
                </div>
              </div>
            ))}
          </div>
